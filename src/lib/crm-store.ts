...
// INITIAL SEED LEADS & CALL HISTORIES
const now = new Date();
const nowISO = now.toISOString();

// Create sample ISO dates relative to current time for realistic overdue/due/upcoming state
const overdueTime1 = new Date(now.getTime() - 45 * 60 * 1000).toISOString(); // 45 mins overdue
const overdueTime2 = new Date(now.getTime() - 15 * 60 * 1000).toISOString(); // 15 mins overdue
const dueNowTime = new Date(now.getTime() + 2 * 60 * 1000).toISOString(); // In 2 mins / Due
const upcomingTime1 = new Date(now.getTime() + 35 * 60 * 1000).toISOString(); // In 35 mins
const upcomingTime2 = new Date(now.getTime() + 120 * 60 * 1000).toISOString(); // In 2 hours

const DEFAULT_LEADS: Lead[] = [
  {
    id: 'lead-1',
    unique_lead_id: 'LD-20260819-00125',
    source: 'INDIAMART',
    source_lead_id: 'IM-881920',
    customer_name: 'ABC Traders (Suresh Gupta)',
    company_name: 'ABC Traders',
    mobile_number: '+91 9876500125',
    alternate_number: '+91 9876500999',
    email: 'suresh@abctraders.com',
    city: 'Delhi',
    state: 'Delhi',
    client_requirement: 'Need 500 meters Cotton Fabric for promotional tote bags',
    product: 'Cotton Canvas 300 GSM',
    enquiry_message: 'Hi, please share bulk pricing for 500m cotton fabric.',
    other_details: { gst_number: '07AAAAA0000A1Z5' },
    lead_received_at: new Date(now.getTime() - 120 * 60 * 1000).toISOString(),
    assigned_user_id: 'user-exec-1',
    assigned_user_name: 'Rahul Verma',
    current_status: 'FOLLOW_UP',
    current_planned_call_at: overdueTime1,
    next_followup_at: overdueTime1,
    last_call_at: new Date(now.getTime() - 180 * 60 * 1000).toISOString(),
    last_call_status: 'FOLLOW_UP',
  ...
  {
    id: 'lead-2',
    unique_lead_id: 'LD-20260819-00126',
    source: 'JUSTDIAL',
    source_lead_id: 'JD-554411',
    customer_name: 'Rahul Enterprises (Vikram Mehta)',
    company_name: 'Rahul Enterprises',
    mobile_number: '+91 9988776655',
    email: 'vikram@rahulenterprises.in',
    city: 'Noida',
    state: 'Uttar Pradesh',
    client_requirement: 'Interested in Solar Panel Mounting Fabric 5KW',
    product: 'Industrial Polyester Fabric',
    enquiry_message: 'Looking for weather resistant fabric supplier.',
    lead_received_at: new Date(now.getTime() - 60 * 60 * 1000).toISOString(),
    assigned_user_id: 'user-exec-2',
    assigned_user_name: 'Amit Patel',
    current_status: 'NOT_REACHABLE',
    current_planned_call_at: overdueTime2,
    next_followup_at: null,
    last_call_at: new Date(now.getTime() - 245 * 60 * 1000).toISOString(),
    last_call_status: 'NOT_REACHABLE',
  ...
  {
    id: 'lead-3',
    unique_lead_id: 'LD-20260819-00127',
    source: 'INDIAMART',
    source_lead_id: 'IM-992200',
    customer_name: 'XYZ Pvt Ltd (Ananya Roy)',
    company_name: 'XYZ Pvt Ltd',
    mobile_number: '+91 9711223344',
    email: 'ananya@xyzpvtltd.com',
    city: 'Gurgaon',
    state: 'Haryana',
    client_requirement: '200 Units Designer Linen Shirts Fabric',
    product: 'Linen Blend Premium',
    enquiry_message: 'Requesting sample swatches to Gurgaon office.',
    lead_received_at: new Date(now.getTime() - 30 * 60 * 1000).toISOString(),
    assigned_user_id: 'user-exec-1',
    assigned_user_name: 'Rahul Verma',
    current_status: 'INTERESTED',
    current_planned_call_at: dueNowTime,
    next_followup_at: dueNowTime,
    last_call_at: new Date(now.getTime() - 150 * 60 * 1000).toISOString(),
  ...
  {
    id: 'lead-4',
    unique_lead_id: 'LD-20260819-00128',
    source: 'JUSTDIAL',
    source_lead_id: 'JD-772299',
    customer_name: 'Ketan Textile Mills',
    company_name: 'Ketan Textiles',
    mobile_number: '+91 9811998877',
    city: 'Surat',
    state: 'Gujarat',
    client_requirement: 'Bulk Denim Fabric 14 Oz - 2000 Meters',
    product: 'Raw Indigo Denim',
    enquiry_message: 'Urgent enquiry for export order.',
    lead_received_at: new Date(now.getTime() - 15 * 60 * 1000).toISOString(),
    assigned_user_id: 'user-exec-3',
    assigned_user_name: 'Priya Singh',
    current_status: 'NEW',
    current_planned_call_at: upcomingTime1,
    next_followup_at: null,
    last_call_at: null,
  ...
  {
    id: 'lead-5',
    unique_lead_id: 'LD-20260819-00120',
    source: 'MANUAL',
    customer_name: 'Shree Ram Garments',
    company_name: 'Shree Ram Garments',
    mobile_number: '+91 9654321098',
    city: 'Jaipur',
    state: 'Rajasthan',
    client_requirement: 'Cotton Silk Blend Fabric for Kurtis',
    product: 'Chanderi Silk',
    lead_received_at: new Date(now.getTime() - 24 * 3600 * 1000).toISOString(),
    assigned_user_id: 'user-exec-1',
    assigned_user_name: 'Rahul Verma',
    current_status: 'CONVERTED',
    current_planned_call_at: null,
    next_followup_at: null,
    last_call_at: new Date(now.getTime() - 4 * 3600 * 1000).toISOString(),
    last_call_status: 'CONVERTED',
    total_call_attempts: 4,
    converted_at: new Date(now.getTime() - 4 * 3600 * 1000).toISOString(),
    deal_amount: 145000,
...
const DEFAULT_CALL_LOGS: CallLog[] = [
  {
    ...
    attempt_number: 1,
    planned_call_at: new Date(now.getTime() - 300 * 60 * 1000).toISOString(),
    actual_call_started_at: new Date(now.getTime() - 295 * 60 * 1000).toISOString(),
    call_duration_seconds: 140,
    call_status: 'INTERESTED',
    remarks: 'Customer interested in 500m cotton fabric. Requested email quotation.',
    next_followup_at: new Date(now.getTime() - 180 * 60 * 1000).toISOString(),
    lead_status_after_call: 'INTERESTED',
    created_at: new Date(now.getTime() - 295 * 60 * 1000).toISOString(),
  },
  {
    id: 'call-log-2',
    lead_id: 'lead-1',
    ...
    user_name: 'Rahul Verma',
    attempt_number: 2,
    planned_call_at: new Date(now.getTime() - 180 * 60 * 1000).toISOString(),
    actual_call_started_at: new Date(now.getTime() - 180 * 60 * 1000).toISOString(),
    call_duration_seconds: 210,
    call_status: 'FOLLOW_UP',
    remarks: 'Quotation sent on WhatsApp & Email. Asked to call back at 4:00 PM after partner approval.',
    next_followup_at: overdueTime1,
    lead_status_after_call: 'FOLLOW_UP',
  ...
  },
  {
    ...
    attempt_number: 1,
    planned_call_at: new Date(now.getTime() - 250 * 60 * 1000).toISOString(),
    actual_call_started_at: new Date(now.getTime() - 245 * 60 * 1000).toISOString(),
    call_duration_seconds: 0,
    call_status: 'NOT_REACHABLE',
    remarks: 'Phone rang completely but no answer on first attempt.',
    next_followup_at: null,
    lead_status_after_call: 'NOT_REACHABLE',
    created_at: new Date(now.getTime() - 245 * 60 * 1000).toISOString(),
  },
];
...
const DEFAULT_FOLLOWUPS: Followup[] = [
  {
    id: 'fol-1',
    lead_id: 'lead-1',
    lead_unique_id: 'LD-20260819-00125',
    customer_name: 'ABC Traders (Suresh Gupta)',
    mobile_number: '+91 9876500125',
    assigned_user_id: 'user-exec-1',
    assigned_user_name: 'Rahul Verma',
    created_from_call_id: 'call-log-2',
    followup_at: overdueTime1,
    followup_status: 'PENDING',
    remarks: 'Quotation sent on WhatsApp & Email. Call after partner approval.',
    created_at: new Date(now.getTime() - 180 * 60 * 1000).toISOString(),
  },
  {
    id: 'fol-2',
    lead_id: 'lead-3',
    lead_unique_id: 'LD-20260819-00127',
    customer_name: 'XYZ Pvt Ltd (Ananya Roy)',
    mobile_number: '+91 9711223344',
    assigned_user_id: 'user-exec-1',
    assigned_user_name: 'Rahul Verma',
    followup_at: dueNowTime,
    followup_status: 'PENDING',
    remarks: 'Sample swatches received. Discuss fabric weight.',
